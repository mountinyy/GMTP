...
import numpy as np
from pyserini.encode import AnceQueryEncoder, AutoQueryEncoder, BprQueryEncoder, QueryEncoder, query_encoder_class_map
...
from pyserini.search.faiss._prf import DenseVectorAncePrf, DenseVectorAveragePrf, DenseVectorRocchioPrf
from pyserini.search.faiss._searcher import BinaryDenseFaissSearcher, FaissSearcher
...
def define_dsearch_args(parser):
    ...
    parser.add_argument(
        "--l2-norm", action="store_true", help="whether to normalize embedding", default=False, required=False
    )
    parser.add_argument(
        "--tokenizer", type=str, metavar="name or path", required=False, help="Path to a hgf tokenizer name or path"
    )
    parser.add_argument(
        "--encoded-queries",
        type=str,
    ...
    parser.add_argument(
        "--pca-model", type=str, metavar="path", required=False, default=None, help="Path to a faiss pca model"
    )
    parser.add_argument(
        "--device",
        type=str,
    ...
    parser.add_argument(
        "--query-prefix", type=str, metavar="str", required=False, default=None, help="Query prefix if exists."
    )
    parser.add_argument(
        "--searcher", type=str, metavar="str", required=False, default="simple", help="dense searcher type"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--binary-hits", type=int, metavar="num", required=False, default=1000, help="Number of binary hits."
    )
    ...
    parser.add_argument(
        "--max-passage", action="store_true", default=False, help="Select only max passage from document."
    )
    parser.add_argument(
        "--max-passage-hits",
        type=int,
    ...
    parser.add_argument(
        "--batch-size", type=int, metavar="num", required=False, default=1, help="search batch of queries in parallel"
    )
    parser.add_argument(
        "--threads", type=int, metavar="num", required=False, default=1, help="maximum threads to use during search"
    )
    ...
    query_iterator = get_query_iterator(args.topics, TopicsFormat(args.topics_format))  # Iterator: (test_id, query)

    topics = query_iterator.topics  # Dict: {"test_id": {"title": query}}
    ...
    # 여기까지 topics는 신경 안써도 될 듯. topic에 관계없이 그냥 encoder, encdoer_class에 따른 encoder 리턴함.
    if args.pca_model:
    ...
    if args.prf_depth > 0 and type(searcher) == FaissSearcher:
        ...
        # ANCE-PRF is using a new query encoder, so the input to DenseVectorAncePrf is different
            ...
            prf_query_encoder = AnceQueryEncoder(
                encoder_dir=args.ance_prf_encoder, tokenizer_name=args.tokenizer, device=args.device
            )
    ...
    for index, (topic_id, text) in enumerate(tqdm(query_iterator, total=len(topics.keys()))):
            ...
            if (index + 1) % args.batch_size == 0 or index == len(topics.keys()) - 1:
                if PRF_FLAG:
                    q_embs, prf_candidates = searcher.batch_search(
                        batch_topics, batch_topic_ids, k=args.prf_depth, return_vector=True, **kwargs
                    )
                    # ANCE-PRF input is different, do not need query embeddings
                    if args.prf_method.lower() == "ance-prf":
                    ...
                    results = searcher.batch_search(
                        prf_embs_q, batch_topic_ids, k=args.hits, threads=args.threads, **kwargs
                    )
        ...
        for topic, hits in results:
            ...
            # output_writer.write(topic, hits) # topic : query_id, hits: top-k개의 DenseSearchResult(docid="doc_id", score=float)
            item = {"query_id": topic, "topk": [[item.docid, float(item.score)] for item in hits]}
